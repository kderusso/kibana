...
/**
 * Retries transient network or availability errors from Elasticsearch with an
 * exponential backoff. Only wrap operations that are idempotent and can be
 * safely executed more than once (e.g. template upserts). Non-retryable errors
 * (bad request, mapping errors, etc.) are rethrown immediately.
 */
export const retryTransientEsErrors = async <T>(
  ...
  try {
    ...
    if (attempt < MAX_ATTEMPTS && isRetryableEsClientError(error)) {
      ...
      const retryDelaySec = Math.min(Math.pow(2, retryCount), 64); // 2s, 4s, 8s, 16s, 32s, 64s...
