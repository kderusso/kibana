...
describe('installAiIndexTemplates', () => {
  ...
  it('marks every template as managed with a version', async () => {
    await installAiIndexTemplates({ esClient, logger });
    ...
    for (const name of [
      ...
      expect(getComponent(name)?._meta).toEqual(MANAGED_META);
    }

    for (const name of [AI_INDEX_IDX_TEMPLATE, AI_INDEX_DS_TEMPLATE]) {
      expect(getIndexTemplate(name)?._meta).toEqual(MANAGED_META);
    }
  });
